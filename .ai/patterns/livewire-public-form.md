...
**У `phone` правила из `FormField::rules` не применяются вообще** — ветка
стоит выше `parseExtraRules()`, правила ровно `[required|nullable,
PHONE_RULE]`. Формат задан маской, любое добавленное правило с ней
взаимоисключающе: так и сломался прод (баг #43 — `regex:/^(?:\+7|8|7)\d{10}$/`
из админки поверх маски, 12 форм не отправлялись; правило жило в данных, не в
коде, поэтому тесты его не видели). Редактор `rules` у `phone` в
`FieldsRelationManager` скрыт — но полагаться на это нельзя, скрытые поля
Filament не дегидрирует, значение в БД остаётся.

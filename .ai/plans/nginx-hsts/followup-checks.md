...
### A. Подъём stage `max-age`

Сейчас `max-age=300` (5 минут). Если TLS на stage стабилен и
`certbot renew` за это время отработал (или следующий уже близко) —
можно поднять. Логика подъёма:

- **300 → 3600** (1 час) — консервативно, попробуй сначала это.
- **3600 → 86400** (1 сутки) — если ок неделю на 1 часе.
- **86400 → 31536000** (год, боевое) — не сразу; только если убеждены,
  что stage-TLS не сломается.

**Важно:** при подъёме `max-age` на stage — добавить HSTS-дубли в
`/vendor/livewire/`, `/vendor/filament/`, статику stage-конфига
(сейчас там нет, потому что при 5-минутном max-age не имело смысла).

### B. Подача на hstspreload.org — **пока не подаём**

Директива `preload` убрана из заголовка. Чтобы подать:
1. Вернуть `preload` в конце HSTS-строки во всех 5 местах prod-конфига.
2. Убедиться, что все поддомены (`stage.` и любые будущие) — на HTTPS.
3. Подать на https://hstspreload.org.

**Не делать это через followup — это отдельное решение.** После
подачи откат занимает 6-12 месяцев (не через `max-age`, а через
сборки браузеров).
...
### D. Глобальный `nginx.conf` cleanup

В `/etc/nginx/nginx.conf` до сих пор `ssl_protocols TLSv1 TLSv1.1 TLSv1.2 TLSv1.3;`.
Наши server-level настройки перекрывают, но default остаётся
неправильный. Если есть другие сайты на сервере (не только niipigrad) —
они принимают TLSv1/1.1.

```
ssh ... 'ls /etc/nginx/sites-enabled/ && grep -H ssl_protocols /etc/nginx/sites-enabled/*'
```

Если только `niipigrad-prod`, `niipigrad-stage`, `default` — можно
безопасно поправить nginx.conf на `TLSv1.2 TLSv1.3`. Если что-то ещё —
осторожнее.

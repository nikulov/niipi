...
[bugs.md](bugs.md) (#22), [bug-report-2026-08-04.md](bug-report-2026-08-04.md)
...
- [ ] 🔥 **#41 ГОРЯЧИЙ** Почта не уходит с прода вообще: SMTP рвётся на
      проверке TLS — `mail.niipigrad.ru` отдаёт чужой сертификат
      `CN=*.hosting.reg.ru` (сервер reg.ru `server319.hosting.reg.ru`).
      Сломалось 06.08 между 15:07 (05.08, последнее ушедшее письмо) и 15:06.
      Заявка 1108 уже в `failed_jobs`. Чинится в `.env`:
      `MAIL_HOST=server319.hosting.reg.ru` + `config:clear` + рестарт воркера;
      параллельно — заявка в reg.ru на SSL для `mail.niipigrad.ru`.
      `app/Jobs/SendFormSubmissionEmails.php:61`
...
- [ ] **#40** Девять потерянных писем по заявкам висят в `failed_jobs`
      (7 таймаутов job + 1 коннект-таймаут 03.03 + 1 от #41 за 06.08).
      Эти письма не ушли.

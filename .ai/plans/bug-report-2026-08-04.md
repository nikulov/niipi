...
**Дополнено 2026-08-21 по итогам репро** (разбор —
[public-form-crashes](public-form-crashes/README.md)): механизм найден —
неудачная запись файла отдаёт `200 {"paths":[""]}`, и пустой путь уезжает в
`_finishUpload` как есть. Сами 34 байта в это не укладываются: ответ «ничего не
сохранилось» весит 12–26 байт, ответ с реальным путём — от 52. Падает при этом
пофайловое правило `uploads.<name>.*` → `max`, а не контейнерное, как
предполагалось выше: `mimetypes` отрабатывает раньше и валится тихо, без
записи в лог.
...
**Перепроверено 20.08.2026 — бот на месте**, последний запрос
`20/Aug/2026:11:46:32`, та же подсеть, тот же профиль нагрузки. Не баг, но
отбить его на уровне nginx стоит: пункт заведён в «Active» в
[plan.md](plan.md).

...
Инвентаризация на 2026-07-13 (`76 passed, 3 failed`). Замер по файлам, а
не по строкам (line coverage не собирался). Текущий прогон —
`291 passed` на 2026-08-04; таблица ниже с тех пор не пересобиралась,
дописанное покрытие см. в разделе «Добавлено после инвентаризации».
...
## Добавлено после инвентаризации

- **Блоки:** `RelatedThematicRendererTest` — ветвление Post/Project,
  override `categoryIds`, исключение текущей записи, кламп `limit`.
- **Запросы:** `NewsQueryTest`/`ProjectsQueryTest` — фильтр по
...
- **Формы, разметка** (`PublicFormTest`): `selected`/`checked` в HTML
  совпадают со state Livewire; плейсхолдер проходит валидацию у
  опционального поля и роняет её у обязательного.

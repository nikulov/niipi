...
| 5 | OPcache / `$realpath_root` | закрыт 10.08, см. ниже |
...
Проверка «новый релиз виден без `opcache_reset`» требует деплоя, а деплоев
с 15.07 так и не было: на 10.08 `current` по-прежнему смотрит на
`releases/20260715142417`. Ждать выкатки ради галочки смысла нет — класс
отказа закрыт независимо от неё:

- `$realpath_root` стоит в конфиге (`fastcgi_param SCRIPT_FILENAME`/
  `DOCUMENT_ROOT`), каждый релиз получает собственный ключ кэша;
- на сервере `opcache.validate_timestamps = On`, `revalidate_freq = 2` —
  OPcache и так перечитывает файлы по mtime раз в две секунды.

То есть залипание старого кода после переключения симлинка невозможно даже
без `$realpath_root`; он тут за корректность, а не за спасение от катастрофы.
Если первая же выкатка всё-таки покажет старую версию — смотреть `phpinfo`
на серверный `SCRIPT_FILENAME`.

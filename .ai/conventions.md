...
## Ссылки в блоках хранятся сырыми — всегда с ведущим `/`

`x-buttons.btn` выводит `href="{{ $url }}"` без обработки, а
`UrlInput::normalize()` обслуживает **только** suffix-кнопку «открыть в новой
вкладке» — в сохраняемое состояние он не вмешивается. Что редактор ввёл, то и
уедет в `href`.

Поэтому внутренний адрес в `btnUrl` обязан начинаться со слэша. Без него
ссылка относительная и разрешается от текущего пути:

| Откуда              | `href="projects"` даёт        |
| ------------------- | ----------------------------- |
| `/{slug}` (страница) | `/projects` — случайно работает |
| `/projects/{slug}`  | `/projects/{slug}/projects` — 404 |

На 2026-08-27 в блоках новостей и проектов все `btnUrl` со слэшем. У `Page`
осталось ~32 кнопки без него (`services`, `news`, `projects`, `personal`,
`license`, `law`, `a_concept`) — сейчас не бьются только потому, что страницы
лежат на одном уровне; вложи страницу глубже — сломаются разом.

Ключи `iconUrl`/`imageUrl`/`bgImageUrl`/`url` в блоках-картинках — это пути на
публичном диске (`images/…`), к ним требование слэша не относится.

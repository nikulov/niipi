...
- **Форматтер:** Prettier + `prettier-plugin-blade` + `prettier-plugin-tailwindcss`.
  Запуск: `npm run format` (или через Sail). Это `prettier --write .` — гонит
  по **всему репозиторию**, включая `.ai/*.md` и чужие blade. Правишь пару
  файлов — форматируй их точечно: `npx prettier --write <файл>`.

@php
    $isModal = (bool) ($viewData["isModal"] ?? false);
    ob_start();
@endphp
...
<div class="py-inner-section-y bg-background-dark px-20">
    @if ($submitted)
        <div class="text-text dark:text-white-dark flex flex-col items-center justify-center p-6">
            ...
            @if ($isModal)
                <x-buttons.btn-add @click.prevent="close" type="" label="Закрыть" class="mx-auto mt-6" />
            @endif
        </div>
    @else
...
<section
    class="{{ $isModal ? "" : "px-inner-section-x my-inner-section-y" }} relative mx-auto w-full max-w-1242"
    @if ($isModal)
        x-data="{
            open: false,
            hash: '#{{ $form->slug }}',
    ...
    >
        <div @class([$isModal ? "absolute inset-0 flex w-full items-end p-4 sm:items-center sm:justify-center" : ""])>
            <div
                @class([
                    "mx-auto max-w-225",
                    "bg-background-light dark:bg-background-dark relative w-full" => $isModal,
                ])
            >
                @if ($isModal)

<?php

return [
    'title' => 'Title',
    'description' => 'Description',
    'content' => 'Content',
    'image' => 'Image',
    'status' => 'Status',
    'pinned_projects' => 'Pinned projects',
    'pinned_projects_hint' => 'Shown first, drag them to change their order. Remaining slots are filled automatically. Select nothing to fill the whole block automatically.',
    'sort_order' => 'Order',
    'sort_order_hint' => '1 comes first, then 2, 3… Empty or 0 goes last, sorted by publication date.',
    'published_at' => 'Published at',
    'meta_title' => 'Meta title',
    'meta_keywords' => 'Meta keywords',
    'meta_description' => 'Meta description',
    'category' => 'Category',
    'categories' => 'Categories',
    'posts' => 'Posts',
    'add_post' => 'Add post',
    'edit_post' => 'Edit post',
    'delete_post' => 'Delete post',
    'view_post' => 'View post',
    'add_category' => 'Add category',
    'edit_category' => 'Edit category',
    'delete_category' => 'Delete category',
    'view_category' => 'View category',
    'name' => 'Name',
    'slug' => 'Slug',
    'action' => 'Action',
    'save' => 'Save',
    'cancel' => 'Cancel',
    'delete' => 'Delete',
    'edit' => 'Edit',
    'add' => 'Add',
    'search' => 'Search',
    'news' => 'News',
    'general' => 'General',
    'seo' => 'SEO',
    'image_upload' => 'Image upload',
    'updated_at' => 'Updated at',
    'created_at' => 'Created at',
    'news_list' => 'News list',
    'alt_text' => 'Alt text',
    'choose_image' => 'Choose image',
    'choose_images' => 'Choose images',
    'alignment' => 'Alignment',
    'paragraph' => 'Paragraph',
    'heading' => 'Heading',
    'quote' => 'Quote',
    'gallery' => 'Gallery',
    'add_block' => 'Add block',
    'search_placeholder' => 'Search title',
    'posts_count' => 'Posts count',
    'settings' => 'Settings',
    'open_page_in_new_tab' => 'Open page in new tab',
    'left' => 'Left',
    'right' => 'Right',
    'center' => 'Center',
    'published' => 'Published',
    'draft' => 'Draft',
    'archived' => 'Archived',
    'active' => 'Active',
    'view' => 'View',
    'heading_size' => 'Heading size',
    'thumbnail' => 'Thumbnail',
    'filter' => 'Filter',
    'created_from' => 'Created from',
    'created_until' => 'Created until',
    'top_section' => 'Top section',
    'main_section' => 'Main section',
    'bottom_section' => 'Bottom section',
    'category_saved_successfully' => 'Category updated successfully',
    'page_saved_successfully' => 'Page updated successfully',
    'post_saved_successfully' => 'Post updated successfully',
    'post_created_successfully' => 'Post created successfully',
    'add_top_block' => 'Add block to top section',
    'add_main_block' => 'Add block to main section',
    'add_bottom_block' => 'Add block to bottom section',
    'add_accordion' => 'Add accordion',
    'add_card' => 'Add card',
    'add_achievement' => 'Add achievement',
    'add_slide' => 'Add slide',
    'type' => 'Type',
    'point' => 'Point',
    'body' => 'Body',
    'cards_block' => 'Cards block with button',
    'cards_block_with_image_title' => 'Cards with image and title',
    'card' => 'Card',
    'tabs_block' => 'Tabs block',
    'tabs' => 'Tabs',
    'modal_block' => 'Modal block',
    'width_full' => 'Full width',
    'id_modal_help' => 'Allowed: lowercase letters, numbers, hyphen, underscore. Must start with a letter.',
    'add_background_image' => 'Add background image',
    'close' => 'Close',
    'email_template_is_empty' => 'Email template is empty',
    'error_message' => 'What went wrong',
    'extra' => 'Extra',
    'mail_skipped_no_admin_recipient' => 'The admin email was not sent: no recipient address in the form settings.',
    'mail_skipped_no_user_email' => 'The user email was not sent: the submission carries no address — the form has no enabled Email field, or it was left empty.',
    'mail_attachments_missing' => 'The user email went out without attachments — these files are not in storage: :files. Check the attachments in the form settings.',
    'extra_help' => 'Additional settings',
    'id' => 'ID',
    'options_help' => 'Options help',
    'options_help_select' => 'Array of options. The first row may act as a placeholder: {"label": "Choose a topic", "value": "", "disabled": true, "default": true} — an empty value is allowed only together with "disabled": true, and such an option keeps HTML5 required working. The "default" flag is honoured only on the first option that carries it.',
    'preview_admin_mail' => 'Admin mail preview',
    'preview_user_mail' => 'User mail preview',
    'send_test_admin_mail' => 'Send test admin mail',
    'send_test_user_mail' => 'Send test user mail',
    'send_to' => 'Send to',
    'test_email_sent' => 'Test email sent',
    'btn_label' => 'Button label',
    'more_details' => 'More details',
    'btn_url' => 'Button URL',
    'url_placeholder' => 'Enter a link',
    'info_block_with_achievements' => 'Info block with achievements',
    'info-block-with-buttons' => 'Info block with buttons',
    'news_block' => 'News block',
    'projects-block' => 'Projects block',
    'projects' => 'Projects',
    'all_news' => 'All news',
    'all-projects' => 'All projects',
    'bg_image' => 'Background image',
    'icon' => 'Icon',
    'icon_alt' => 'Icon alt text',
    'image_alt' => 'Image alt text',
    'alt' => 'Alt text',
    'text' => 'Text',
    'achievements' => 'Achievements',
    'achievement' => 'Achievement',
    'quantity' => 'Quantity',
    'contact-data' => 'Contacts',
    'social' => 'Social',
    'add_social_icon' => 'Add social icon',
    'site-title' => 'Site title',
    'site-description' => 'Site description',
    'admin-email' => 'Admin email',
    'favicon' => 'Favicon',
    'code' => 'Code',
    'code_header' => 'Code in <head>',
    'code_body_top' => 'Code at the beginning of <body>',
    'code_body_bottom' => 'Code at the end of <body>',
    'limit' => 'Limit',
    'category_created_successfully' => 'Category created successfully',
    // Menus (Resources/Menus/Schemas/MenuForm.php)
    'top_menu' => 'Top menu',
    'top_menu_items' => 'Top menu items',
    'add_top_menu_item' => 'Add top menu item',
    'sub_menu_items' => 'Submenu items',
    'add_sub_menu_item' => 'Add submenu item',
    'footer_menu' => 'Footer menu',
    'footer_menu_items' => 'Footer menu items',
    'add_footer_menu_item' => 'Add footer menu item',
    'type_link' => 'Link type',
    'page' => 'Page',
    'custom_url' => 'Custom URL',
    'label' => 'Label',
    // Footer (Resources/Footers/Schemas/FooterForm.php)
    'social_icon' => 'Social icon',
    'url' => 'URL',
    'publications' => 'Publications',
    'projects_list' => 'Projects list',
    'excerpt_project' => 'Excerpt',
    'projects_full' => 'Full list of projects',
    'edit_page' => 'Edit page',
    'delete_page' => 'Delete page',
    'copy' => 'Copy',
    'copy_post' => 'Copy post',
    'copy_post_confirm' => 'Copy post «:title»?',
    'post_copied' => 'Post copied',
    'copy_project' => 'Copy project',
    'copy_project_confirm' => 'Copy project «:title»?',
    'project_copied' => 'Project copied',
    'copy_page' => 'Copy page',
    'copy_page_confirm' => 'Copy page «:title»?',
    'page_copied' => 'Page copied',
    'copy_form' => 'Copy form',
    'copy_form_confirm' => 'Copy form «:name»?',
    'form_copied' => 'Form copied',
    'services_block' => 'Services block',
    'position' => 'Position',
    'primary' => 'Primary',
    'secondary' => 'Secondary',
    'accent' => 'Accent',
    'accent_additional' => 'Accent additional',
    'btn' => 'Button',
    'share_btn' => 'Share + button',
    'show_copy_link' => '“Copy link” button',
    'share_url' => 'Share URL',
    'share_url_hint' => 'Template: {url} — the page address, {title} — its title. For example: https://vk.com/share.php?url={url}&title={title}',
    'svg' => '*.svg only',

    'submission' => 'Submission',
    'submissions' => 'Submissions',
    'status_new' => 'New',
    'status_processing' => 'Processing',
    'status_sent' => 'Sent',
    'status_failed' => 'Failed',
    'form' => 'Form',
    'data' => 'Data',
    'field' => 'Field',
    'value' => 'Value',
    'ip' => 'IP address',
    'user_agent' => 'User agent',
    'email' => 'Email',
    'phone' => 'Phone',
    'invalid_phone' => 'Enter the phone number in the format +7 (999) 123-45-67.',
    'submission_confirmation' => 'Your request has been received',
    'submission_thanks' => 'Thank you! We have received your request and will contact you shortly.',
    'files' => 'Files',
    'file_name' => 'File',
    'mime_type' => 'MIME type',
    'size' => 'Size',
    'download' => 'Download',
    'status_handled' => 'Handled',
    'options' => 'Options',
    'recipient_admin_email' => 'Recipient admin email',
    'required' => 'Required',
    'is_active' => 'Active',
    'field_name_help' => 'Value will be saved in database as "name" attribute',
    'validation_rules' => 'Validation rules',
    'rules_help' => 'Additional validation rules. Format: JSON object.
                    Example: {"min:3":"","max:255":"","regex:/^[a-z0-9_]+$/i":"Only latin letters and digits"}
                    Empty value uses the default message.
                    A plain list works too: ["min:3","max:255"] — with default messages.
                    Do not include "required" here — it is controlled by the Required toggle.',
    'field_type' => 'Field type',
    'field_label' => 'Field label',
    'field_name' => 'Field name',
    'resend_email' => 'Resend email',
    'display_type' => 'Display type',
    'submissions_count' => 'Submissions count',
    'fields' => 'Fields',
    'pages' => 'Pages',
    'pages_list' => 'Pages list',
    'forms' => 'Forms',
    'forms_list' => 'Forms list',
    'submissions_list' => 'Submissions list',
    'subheading' => 'Subheading',
    'yes' => 'Yes',
    'no' => 'No',
    'field_placeholder' => 'Field placeholder',
    'textarea' => 'Textarea',
    'file' => 'File',
    'select' => 'Select',
    'checkbox' => 'Checkbox',
    'radio' => 'Radio Button',
    'all' => 'All',
    'too_many_attempts' => 'Too many attempts. Please try again in :seconds seconds.',
    'upload_lost' => 'The file was not uploaded. Please attach it again.',
    'upload_broken' => 'The attached file could not be read. Please attach it again.',
    'upload_not_saved' => 'The file could not be saved. Please submit the form again.',
    'send' => 'Send',
    'select_options' => 'Select options',
    'change_theme' => 'Change site theme',
    'website' => 'Website',
    'email_subject' => 'Email subject',
    'email_body' => 'Email body',
    'emails' => 'Emails',
    'email_admin' => 'Admin email',
    'send_admin_mail' => 'Send admin email',
    'email_user' => 'User email',
    'send_user_mail' => 'Send user email',
    'user_mail_attachments' => 'User email attachments',
    'user_mail_attachments_help' => 'Files will be attached to the user email.',
    'success_message' => 'Success message',
    'modal_id' => 'Modal ID',
    'tab' => 'Tab',
    'title_url' => 'Title URL',
    'yandex-map' => 'Yandex Map',
    'form_name' => 'Form name',
    'applicant_type' => 'Applicant type',
    'person' => 'Individual',
    'company' => 'Legal entity',
    'common' => 'Common',
    'new_today' => 'New today',
    'password' => 'Password',
    'password_confirmation' => 'Confirm password',
    'password_requirements_short' => '8+ characters, upper/lowercase, number, symbol.',
    'leave_blank_to_keep_password' => 'Leave blank to keep the current password.',
    'role' => 'Role',
    'admin' => 'Administrator',
    'editor' => 'Editor',
    'viewer' => 'Viewer',
    'email_verified_at' => 'Email verified',
    'send_verification_email' => 'Send verification email',
    'verification_email_sent' => 'Verification email sent',
    'verification_email_failed' => 'Failed to send verification email',
    'user_name' => 'User name',
    'user' => 'User',
    'users' => 'Users',

    'anchor' => 'Anchor',
    'anchor_duplicate' => 'Anchor ":anchor" is already used',

    'related_thematic_label' => 'Related thematic',
    'related_thematic' => 'Related',
    'related_thematic_categories_hint' => 'If empty — the record’s own categories are used.',

    'media_file' => 'Media file',
    'media_files' => 'Media files',
    'media_files_list' => 'Media files',
    'media_upload' => 'Upload',
    'media_file_info' => 'File info',
    'media_type_image' => 'Image',
    'media_type_document' => 'Document',
    'media_type_other' => 'Other',
    'media_used' => 'In use',
    'media_used_in' => 'Used in',
    'media_not_used' => 'Not used',
    'media_usages_count' => 'Usages',
    'media_copy_url' => 'Copy URL',
    'media_url_copied' => 'URL copied',
    'media_confirm_delete_used' => 'File is used in:',
    'media_choose_from_library' => 'Choose from library',
    'media_picker_title' => 'Media library',
    'media_no_files_found' => 'No files found',
    'media_selected' => 'Selected',
];
